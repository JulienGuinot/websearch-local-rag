...
function generateText(size = 200): string {
    return (
        "This is a benchmark document used to test embedding performance.I really want to compare my rag with yours ".repeat(
            Math.ceil(size / 60)
        )
    );
}
...
async function runBenchmark() {
    ...
    for (const batchSize of BATCH_SIZES) {
        const texts = Array.from({ length: TOTAL_TEXTS }, () =>
            generateText()
        );
