...
export class OllamaService {
  ...
  async generateResponse(query: string, context: Chunk[]): Promise<string> {
  ...
  private async _ensureModelExists(): Promise<void> {
    try {
      ...
      const embeddingModelExists = models.some(
        (model) =>
          model === this.config.embeddingModel ||
          model.startsWith(this.config.embeddingModel + ":")
      );

      if (!embeddingModelExists) {
        throw new Error(
          `Modèle d'embedding '${this.config.embeddingModel}' non installé. Installez-le avec: ollama pull ${this.config.embeddingModel}`
        );
      }
    } catch (error: any) {
  ...
  private _buildUserPrompt(query: string, context: Chunk[]): string {
    const contextText = context
      .map(
        (chunk, index) =>
          `[Source ${chunk.metadata.title} ${index + 1}]\n${chunk.content}`
      )
      .join("\n\n---\n\n");
